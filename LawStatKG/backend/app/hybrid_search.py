...
# ═══════════════════════════════════════════════════════════════
# Build indexes from KG data (BM25 + maps + embeddings)
# CHANGED: Wrapped in try/except so service starts even without Neo4j
# ═══════════════════════════════════════════════════════════════
...
try:
    ...
    print(f"WARNING: Could not connect to Neo4j: {_neo4j_err}")
    print("Starting with empty sections - search will return no results until Neo4j is available.")
...
def hybrid_strict_search(
    ...
    - Otherwise: BM25 gate + token overlap + LegalBERT rerank
    """
    # CHANGED: Return empty if Neo4j data not loaded
    if not ALL_SECTIONS or BM25 is None or MODEL is None:
        return []
